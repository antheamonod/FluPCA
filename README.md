...
* `BHV_PCA_plot.R` produces figures that include the locus of weighted means (second principal component) calculated with respect to the BHV metric (LFM) with projected points, as well as plots of the tree topologies of vertices making up the LFM and the tree topologies of the projected trees
...
The `Trees_R_squared.R` script calculates and compares the percentage of variance explained (R^2) between the two methods, BHV and tropical PCA.  These values are reported in Table 1 in the main text.
